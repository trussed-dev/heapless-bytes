...
mod storage {
    ...
    pub trait BytesStorageSealed: VecStorage<u8> {
        fn as_byte_view<LenT: LenType>(this: &BytesInner<Self, LenT>) -> &BytesView<LenT>
        where
            Self: BytesStorage;
        fn as_byte_mut_view<LenT: LenType>(
            this: &mut BytesInner<Self, LenT>,
        ) -> &mut BytesView<LenT>
        where
            Self: BytesStorage;
    }
...
pub struct BytesInner<S: BytesStorage + ?Sized, LenT: LenType> {
    bytes: VecInner<u8, LenT, S>,
}

pub type Bytes<const N: usize, LenT = usize> = BytesInner<OwnedBytesStorage<N>, LenT>;
pub type BytesView<LenT = usize> = BytesInner<ViewBytesStorage, LenT>;
...
impl<S: BytesStorage + ?Sized, LenT: LenType> Eq for BytesInner<S, LenT> {}
impl<S: BytesStorage + ?Sized, LenT: LenType> Ord for BytesInner<S, LenT> {
...
impl<S: BytesStorage + ?Sized, LenT: LenType> BytesInner<S, LenT> {
...
impl<S: BytesStorage + ?Sized, LenT: LenType> Debug for BytesInner<S, LenT> {
...
impl<S: BytesStorage + ?Sized, LenT: LenType> AsRef<[u8]> for BytesInner<S, LenT> {
    fn as_ref(&self) -> &[u8] {
        &self.bytes
    }
}

impl<S: BytesStorage + ?Sized, LenT: LenType> AsMut<[u8]> for BytesInner<S, LenT> {
    fn as_mut(&mut self) -> &mut [u8] {
        &mut self.bytes
    }
}

impl<S: BytesStorage + ?Sized, LenT: LenType> Deref for BytesInner<S, LenT> {
    type Target = [u8];
...
impl<S: BytesStorage + ?Sized, LenT: LenType> DerefMut for BytesInner<S, LenT> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.bytes
    }
}

impl<Rhs, S: BytesStorage + ?Sized, LenT: LenType> PartialEq<Rhs> for BytesInner<S, LenT>
where
    Rhs: ?Sized + AsRef<[u8]>,
{
...
impl<Rhs, S: BytesStorage + ?Sized, LenT: LenType> PartialOrd<Rhs> for BytesInner<S, LenT>
where
    Rhs: ?Sized + AsRef<[u8]>,
{
...
impl<S: BytesStorage + ?Sized, LenT: LenType> Hash for BytesInner<S, LenT> {
...
impl<'a, S: BytesStorage + ?Sized, LenT: LenType> IntoIterator for &'a BytesInner<S, LenT> {
    type Item = &'a u8;
...
impl<'a, S: BytesStorage + ?Sized, LenT: LenType> IntoIterator for &'a mut BytesInner<S, LenT> {
    type Item = &'a mut u8;
...
impl<S: BytesStorage + ?Sized, LenT: LenType> Serialize for BytesInner<S, LenT> {
...
impl<S: BytesStorage + ?Sized, LenT: LenType> core::fmt::Write for BytesInner<S, LenT> {
